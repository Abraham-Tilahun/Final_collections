...
mysql_select_db('storem');
mysql_select_db('storem',mysql_connect('localhost','root',''))or die(mysql_error());
$result = mysql_query("SELECT * FROM  request_disposal where status='0' ");
...
<td style='height:30px;	color:#000;	font-weight:bold;background-color:white;'><font color='black'><?php
						...
						{
						?>
                       			  <a href="approve1.php?status=<?php echo $row['User_id'];?>" onclick="return confirm('Really you want approve <?php echo $description?>');">
						...
						{
						?>
                       			<a href="approve1.php?status=<?php echo $row['User_id'];?>" onclick="return confirm('Really you want disapproved (<?php echo $description?>)');"> 
