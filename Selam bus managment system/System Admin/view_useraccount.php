...
if(isset($_POST['Activate'])) {
	mysql_query("update account set status='1' where user_id=".$_POST['user_id']);
}
?>
<html>
<head>
	...
	while ($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
		...
		if($row2["status"] == '0') {
			echo "<tr><form action='?' method='post'><input type='text' name='user_id' value='".$row['user_id']."' hidden=true><td>".$row['user_id']."</td><td>".$row['first_name']."</td><td>".$row['last_name']
				."</td><td>".$row['role']."</td><td>".$row2['username']."</td><td><input type='submit' value='Activate' Name='Activate'</td></form></tr>";
		}
