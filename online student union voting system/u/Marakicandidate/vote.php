...
<div id="container">
	   ...
	   ?>
		...
		<div id="menu">
		<?php
include("../Marakicandidate/candidatemenu.php");
		?>
	</div>
				...
				<div id="leftside">
					<div id="applyside">
						<?php
							include("../Marakicandidate/candidatesidemenu.php");
					   ?>	
					</div>    
				</div>
				<div id="content">
...
if ($numrow == 0)
{
	die('Now The Election DATE iS not Started!!!');
}
...
function CountBack(secs) {
  if (secs < 0) {
	...
	window.location = "../Marakicandidate/voteclosed.php"
    return;
  }
...
$start_from = ($page-1) * $per_page;	
	$sql1=mysql_query("select * from user where role='Candidate'and campus='marki' order by fname LIMIT $start_from, $per_page");
	...
	if($row1>0)
	{
				...
				{	
					...
					$userid=$info['userid'];
					$campus=$info['campus'];
					$query=mysql_query("select * from requesttable where username='$username' and campus='$campus'");
			...
			$qry="select * from user where userid='$id' and campus='marki'";
			...
			if(mysql_num_rows($results)>0)
			{
				while ($row = mysql_fetch_array($results)) 
					...
					mysql_query("update requesttable set votstatus='vote' where student_ID='$requestid'");
					echo"You have sucssusfully voted Thank you!";
					}
					else
					{
					mysql_query("INSERT INTO `count` VALUES ('$varvoteid','$fn $ln','$sex','$collage','$dept','$yr','$cgpa','$campus','$infor','1')");	
					mysql_query("INSERT INTO `ballotstore` VALUES ('$id','$varvoteid')");	
					mysql_query("update requesttable set votstatus='vote' where student_ID='$requestid'");
					echo"You have sucssusfully voted Thank you!";
					}
			...
			echo "no data found";
				...
				?>		</fieldset>
				<?php
		$query = "select * from user where role='Candidate'and campus='marki'";
...
//Going to first page
echo "<center> <h3><a href='../Marakicandidate/vote.php?page=1'>".'Privouse Page'."</a> ";
for ($i=1; $i<=$total_pages; $i++) {

echo "<a href='../Marakicandidate/vote.php?page=".$i."'>".$i."</a> ";
};
// Going to last page
echo "<a href='../Marakicandidate/vote.php?page=$total_pages'>".'Next Page'."</a></h3></center> ";
		?>
		    </div>
		<div id="footer">
			<?php
			include("../Marakicandidate/footer.php");
			?>
			<a name="bottom"></a>
	</div>
		<?php
